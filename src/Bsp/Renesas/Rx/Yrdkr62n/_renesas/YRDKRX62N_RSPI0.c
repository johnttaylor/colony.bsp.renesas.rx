...
******************************************************************************/
#include "Bsp/Renesas/Rx/Yrdkr62n/iodefine.h"
...
static void YRDKRX62N_RSPIConfig(void) ;
static void YRDKRX62N_RSPIStart(void) ;
static void YRDKRX62N_RSPIEnd(void) ;
static void YRDKRX62N_RSPITransferOutWord8Bit(uint8_t cData) ;
...
{
#if (APP_CFG_FS_EN > 0u)
    FSDev_SD_SPI_BSP_SPI.Open(1);
    FSDev_SD_SPI_BSP_SPI.SetClkFreq(1, BSP_CPU_PerClkFreq() / 2);
#else
    MSTP(RSPI0) = 0 ;
...
{
#if (APP_CFG_FS_EN > 0u)
    FSDev_SD_SPI_BSP_SPI.Lock(1);
    FSDev_SD_SPI_BSP_SPI.ChipSelEn(1);
#else
    /* Assert chip select */
    PORTC.DR.BIT.B2 = 0 ;
#endif
...
{
#if (APP_CFG_FS_EN > 0u)
    FSDev_SD_SPI_BSP_SPI.ChipSelDis(1);
    FSDev_SD_SPI_BSP_SPI.Unlock(1);
#else
...
#if (APP_CFG_FS_EN > 0u)
    FSDev_SD_SPI_BSP_SPI.Wr(1, &cData, 1);
#else
